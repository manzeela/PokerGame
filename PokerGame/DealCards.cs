...
namespace PokerGame
{
    class DealCards : DeckOfCards
    {
        ...
        {
            //query that stores the sorted card for each hand in an array
            var firstQuery = firstHand.OrderBy(hand => hand.cFace);
            var secondQuery = secondHand.OrderBy(hand => hand.cFace);
            var thirdQuery = thirdHand.OrderBy(hand => hand.cFace);
            var fourthQuery = fourthHand.OrderBy(hand => hand.cFace);

            //storing the sorted cards in the sorted hands for each player
            for (int i = 0; i < firstQuery.ToList().Count; i++) {
                sortedFirstHand[i] = (firstQuery.ToList())[i];
            }

           
            for (int i = 0; i < secondQuery.ToList().Count; i++)
            {
                sortedSecondHand[i] = (secondQuery.ToList())[i];
            }

           
            for (int i = 0; i < thirdQuery.ToList().Count; i++)
            {
                sortedThirdHand[i] = (thirdQuery.ToList())[i];
            }

           
            for (int i = 0; i < fourthQuery.ToList().Count; i++)
            {
                sortedFourthHand[i] = (fourthQuery.ToList())[i];
            }
        }
        ...
        private void displayCards()
        {
            ...
            for (int i = 0; i < 5; i++) 
            {
                Console.WriteLine(sortedFirstHand[i].cFace + " of " + sortedFirstHand[i].cSuit);
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                Console.WriteLine(sortedSecondHand[i].cFace + " of " + sortedSecondHand[i].cSuit);
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                Console.WriteLine(sortedThirdHand[i].cFace + " of " + sortedThirdHand[i].cSuit);
            }
            ...
            for (int i = 0; i < 5; i++)
            {
                Console.WriteLine(sortedFourthHand[i].cFace + " of " + sortedFourthHand[i].cSuit);
            }
        }
        ...
        public void evaluateHands() {
            ...
            EvaluateHands firstHandEvaluator = new EvaluateHands(sortedFirstHand);
            EvaluateHands secondhandEvaluator = new EvaluateHands(sortedSecondHand);
            EvaluateHands thirdHandEvaluator = new EvaluateHands(sortedThirdHand);
            EvaluateHands fourthHandEvaluator = new EvaluateHands(sortedFourthHand);
